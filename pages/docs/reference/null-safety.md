...
第三种选择是为 NPE 爱好者准备的: the not-null assertion operator (`!!`) converts any value to a non-null
type and throws an exception if the value is null.我们可以写 `b!!` ，这会返回一个非空的 `b` 值
